...
services_router_gmail = APIRouter(prefix="/services", tags=["Services"])
google_redirect_uri = 'http://localhost:8080/api/services/callback/google/gmail'
...
@services_router_gmail.get(
    "/google/gmail",
    summary="Request autorizathion to google gmail access data",
)
...
@services_router_gmail.get("/callback/google/gmail")
async def authorize_google_access_callback(request: Request):
    ...
    token = await user_respository.update_service_access_token(user.id, auth_token, "google_gmail_token")
    return token
