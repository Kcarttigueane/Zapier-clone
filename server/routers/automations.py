...
async def get_automation(automation_id: PyObjectId, current_user = Depends(get_current_user)):
    automation_repository = AutomationRepository(current_user.id)
...
async def update_automation(automation_id: PyObjectId, automation: AutomationCreate, current_user = Depends(get_current_user)):
    automation_repository = AutomationRepository(current_user.id)
...
async def delete_automation(automation_id: PyObjectId, current_user = Depends(get_current_user)):
    automation_repository = AutomationRepository(current_user.id)
