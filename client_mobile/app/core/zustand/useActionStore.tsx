...
type ActionState = {
  ...
  actionsAssociatedToService: ActionModelDTO[];
...
type ActionActions = {
  ...
  fetchActionsByServiceId: (serviceId: ServiceModelDTO['id']) => Promise<void>;
};

const initialState: ActionState = {
  actions: [],
  actionsAssociatedToTrigger: [],
  actionsAssociatedToService: [],
...
const useActionStore = create<ActionState & ActionActions>()(set => ({
  ...
  fetchActionsByServiceId: async (serviceId: ServiceModelDTO['id']) => {
    set({ isActionsLoading: true });
    try {
      const accessToken = await getAccessToken();
      if (!accessToken) {
      ...
      const response = await apiV2.get(`/actions/service/${serviceId}`, {
        params: {
          service_id: serviceId,
        },
        headers: getApiHeaders(accessToken),
  ...
  clearActions: () => set(() => ({ actions: [], actionsAssociatedToTrigger: [], actionsAssociatedToService: [] })),
}));
